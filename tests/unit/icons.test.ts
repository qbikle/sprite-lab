...
describe('icon registry', () => {
  ...
  it('derives pairs exactly', () => {
    ...
    expect(GLYPH_ROWS['flip-y']).toEqual(transpose(GLYPH_ROWS['flip-x']));
    expect(GLYPH_ROWS['sym-y']).toEqual(transpose(GLYPH_ROWS['sym-x']));
    expect(GLYPH_ROWS['caret-right']).toEqual(transpose(GLYPH_ROWS['caret-down']));
  });
});
